...
- **Rule 1 (Context Initialization):** Step Zero: You MUST use the `view_file` tool to read this exact configuration file and your `map_[agentname]_domain.md` as your absolute first action upon awakening. Do not execute any other terminal commands until your rules are loaded. If your localized `map_[agentname]_domain.md` does not exist, you MUST create it autonomously before proceeding.
